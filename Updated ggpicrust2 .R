...
ko_abundance_merg <- merge(ko_abundance_a, ko_abundance_i) #merge -- WORKED!
...
# Generate Heatmap !!! NOT WORKING
metacycfeature_with_p_0.05 <- metacyc_daa_annotated_results_df %>% filter(p_adjust < 0.05)
pathway_heatmap(abundance = metacyc_abundance_filtered %>% filter(feature %in% metafeature_with_p_0.05$feature) %>% column_to_rownames("function"), metadata = filtered_metadata_cmb, group = "sex")
significant_results_metacyc <- subset(metacyc_daa_annotated_results_df, p_adjust < 0.05)
expression_data <- significant_results_metacyc[complete.cases(significant_results_metacyc), ] # Remove NA
pheatmap(
  expression_data,
  cluster_rows = TRUE,
  cluster_cols = TRUE,
  main = "Expression Heatmap",
  annotation_col = significant_results_metacyc$p_adjust,  # Use adjusted p-values for annotation
  annotation_colors = list(p_adjust = c("white", "blue"))  # Customize colors
)
  ## ERROR!!!: Error in `filter()`:
    ℹ In argument: `feature %in% metafeature_with_p_0.05$feature`.
    Caused by error in `h()`:
    ! error in evaluating the argument 'x' in selecting a method for function '%in%': object 'feature' not found
...
# Generate heatmap $$!!!!!!!!!
feature_with_p_0.05_metacyc <- metacyc_daa_annotated_results_df %>% 
  filter(p_adjust < 0.05) # Filter features with p < 0.05
heatmap_metacyc <- pathway_heatmap(
  abundance = metacyc_abundance_filtered %>%
    right_join(
      metacyc_daa_annotated_results_df %>% select(all_of(c("feature", "description"))),
      by = c("function" = "feature")
    ) %>%
    filter(description %in% feature_with_p_0.05_metacyc$feature) %>%
    select(-"function") %>%
...
feature_with_p_0.05_metacyc_location <- metacyc_daa_annotated_results_df_location %>% 
  filter(p_adjust < 0.05) # Filter features with p < 0.05
heatmap_metacyc_location <- pathway_heatmap(
                              abundance = metacyc_abundance_filtered_location %>%
                                right_join(
                                  metacyc_daa_annotated_results_df_location %>% select(all_of(c("feature", "description"))),
                                  by = c("function" = "feature")
                                ) %>%
                                filter("function" %in% feature_with_p_0.05_metacyc_location$feature) %>%
                                select(-"function") %>%
                                column_to_rownames("description"),
                              metadata = metadata_cmb,
                              group = "cohort"
                            )
  ### ERROR!!!: Error in dimnames(x) <- dn : 
    length of 'dimnames' [2] not equal to array extent
